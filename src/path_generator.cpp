...
//        std::cout << "[PathGenerator] _bpd_decompositionCells count is " << _bpd_decompositionCells->count() << std::endl;
        ...
        _configurePathIntoCell(_ordering, _startEndPointsIntoCell);
        /*auto iter = _pathIntoCell.begin();
        while(iter != _pathIntoCell.end()) {
            _qDebugPrintPath(iter.value());
            ++iter;
        }*/
        // формируем соединения между cell
...
QList<QList<QPointF>> PathGenerator::_improvePathRespectCell(const QList<QList<QPointF>>& resPointList, const QPolygonF& cell){
    ...
    for(int i = 0; i < resPointList.count() - 1; ++i){
        ...
        auto currLineLast = resPointList[i][1];
        auto nextLineFirst = resPointList[i+1][0];
        auto lineCheck = QLineF(currLineLast, nextLineFirst);
        if(extraDangerPointsRoutine(temp, lineCheck, orderExtraPolyline))
            for(int j = 1; j < orderExtraPolyline.count(); ++j)
                res[i].append(orderExtraPolyline[j]);
    }
...
QList<QList<QPointF>> PathGenerator::getPath() {
    QList<QList<QPointF>> res = {};

    if(_holes != nullptr) {
        int counter = 0;
        for (; counter < _pathConnectionLines.count(); ++counter) {
            res.append(_pathIntoCell[_ordering[counter]]);
            res.append(_pathConnectionLines[counter]);
        }
        res.append(_pathIntoCell[_ordering[counter]]);
    }
    else
        res.append(_orientedPathSimpl);

    return res;
}

void PathGenerator::_configurePathIntoCell(QVector<const QPolygonF*>& order, QVector< QPair<QPointF, QPointF> >& flP)
{
    order.clear();
    flP.clear();
