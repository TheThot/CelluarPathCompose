...
Window {
    ...
    Rectangle {
        ...
        // Канвас для рисования Simpl poly
        Canvas {
            id: canvas1
            anchors.fill: parent
        ...
        // Канвас для рисования Poly with holes
        Canvas {
            id: canvas2
            anchors.fill: parent
            renderTarget: Canvas.FramebufferObject
            ...
            onPaint: {
                ...
                // Рисуем исходный полигон
                drawOriginalPolygonWithHoles(ctx)
            ...
            function drawOriginalPolygonWithHoles(ctx) {
                ...
                for (var onePoly of holesArray) {
                    ctx.beginPath()
                    ctx.moveTo(onePoly[0].x, onePoly[0].y)
                    for(i = 1; i < onePoly.length; i++){
    ...
    Connections {
        target: decomposer
        function onDecompositionCellsChanged() { myToolbar.buttonsVisibleProp ? canvas1.requestPaint() : canvas2.requestPaint() }
        function onOrientedRectChanged() { myToolbar.buttonsVisibleProp ? canvas1.requestPaint() : canvas2.requestPaint() }
        function onOriginalPolygonChanged() { myToolbar.buttonsVisibleProp ? canvas1.requestPaint() : canvas2.requestPaint() }
        function onSweepAngleChanged() { myToolbar.buttonsVisibleProp ? canvas1.requestPaint() : canvas2.requestPaint() }
        function onShowDecompositionChanged() { myToolbar.buttonsVisibleProp ? canvas1.requestPaint() : canvas2.requestPaint() }
        function onShowOrientedRectChanged() { myToolbar.buttonsVisibleProp ? canvas1.requestPaint() : canvas2.requestPaint() }
    }
